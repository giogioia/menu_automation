            ...
            try:
               ...
               print(f'There was a problem while searching for store {partner} on Admin.\nPlease try again. (If problem persists, close bot and try again)')
                ...
                if df_admin.index.size < 1:
                    ...
                    df_admin = df_admin[['name','cityCode','id']]
                    print(df_admin)
                    break
...
def fire_menu():
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for j in df_admin.index:
            executor.submit(menu_requests, j)
...
if __name__ == '__main__':
    ...
    print(df_admin)
    print(f'\nDone!\nSuccessfully exported {partner}-{store_to_export} to {len(df_admin.index)} stores in {round(finish-start,2)} seconds')
    time.sleep(10)
