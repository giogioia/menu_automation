...
def get_prod_externalId(phantom_dic, sbre, access_token):
...
def download_images():
    ...
    try: 
        ...
        with multiprocessing.Manager() as manager: 
            ...
            for nu in df_prods.index[:5]:
                x +=1
                process =  multiprocessing.Process(target = fire_download, args = [nu,x,l,df_prods,image_path])
                process.start()
                processes.append(process)
            for process_django in processes:
                process_django.join()
            im_mod = list(l)
        if len(im_mod) == 0: print('\nNo new image to dowload')
        else: print(f'\nImages folder of {store_name} updated')
        
def fire_download(nu,x,l,df_prods, image_path):
